...
/*
  ...
  Plugin URI: http://www.codvis.com
  ...
  Author: Caspio
  Version: 1.8
  Author URI: http://www.codvis.com
 */
...
function caspio ( $atts = NULL, $content = '' ) {
	...
	if ( ($embed = $atts['embed']) && ($app_key = $atts['key']) ) {
		...
		return '<script type="text/javascript" src="'.esc_attr($embed).'/scripts/'.esc_attr($subdomain_deployment ? 'embed' : 'e1').'.js"></script>'
				...
				.'<div id="cxkg"><a href="'.esc_attr($embed).'/dp.asp?AppKey='.esc_attr($app_key).'">Click here</a> to load this Caspio <a href="http://www.codvis.com" title="Online Database">Online Database</a>.</div>';
